...
export const createReview = async (req: Request, res: Response) => {
  ...
  try {
    ...
    // ✅ Popular el usuario antes de devolver la respuesta
    const populated = await Review.findById(review._id)
      .populate('user', 'firstName lastName')
      .lean()

    return res.status(201).json({ review: populated }) // 👈 se devuelve con los datos del usuario
  } catch (e) {
...
export const listMyReviews = async (req: Request, res: Response) => {
  try {
    ...
    const reviews = await Review.find({ user: userId })
      ...
      .populate('user', 'firstName lastName') // ✅ mostrar nombre del usuario
      .sort({ createdAt: -1 })
      .lean()
...
export const listByMovie = async (req: Request, res: Response) => {
  try {
    ...
    const reviews = await Review.find({ movieId: String(movieId) })
      .populate('user', 'firstName lastName') // ✅ corregido aquí
      .sort({ createdAt: -1 })
      .lean()
...
export const updateReview = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const updated = await Review.findById(id)
      .populate('user', 'firstName lastName') // Showing the user's name
      .lean()
